...
import { deleteCandidateById } from 'apiSdk/candidates';
...
function JobViewPage() {
  ...
  const candidateHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('candidate', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/candidates/create?job_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.candidate?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/candidates/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            <Td>{record.email}</Td>
                            <Td>
                              {hasAccess('candidate', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/candidates/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('candidate', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    candidateHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
